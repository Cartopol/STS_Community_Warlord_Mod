...
public class ReflectionUtils {
    ...
    public static <T> ArrayList<Class<T>> findAllConcreteWarlordModClasses(ClassFilter classFilter)
    {
        try {
            ClassFinder finder = new ClassFinder();
            URL url = WarlordMod.class.getProtectionDomain().getCodeSource().getLocation();
    ...
    public static <T> ArrayList<Class<T>> findAllConcreteWarlordModSubclasses(Class<T> baseJorbsModClass) {
        return findAllConcreteWarlordModClasses(new SubclassClassFilter(baseJorbsModClass));
    }

    // Note: only works for base classes defined within our own jar (limitation of ClassFinder)
    // Requires each class have a no-arg constructor
    public static <T> List<T> instantiateAllConcreteWarlordModSubclasses(Class<T> baseWarlordModClass) {
        ArrayList<Class<T>> classes = findAllConcreteWarlordModClasses(new SubclassClassFilter(baseWarlordModClass));
